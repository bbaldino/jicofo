...
public class JibriSession
{
    ...
    /**
     * The session ID for this JibriSession.  This ID is used to communicate
     * with the web so that it can receive status updates for a particular
     * JibriSession.  It is generated by Jicofo and given to Jibri, so that
     * when Jibri joins the web call it can be matched to the request
     * made to Jicofo
     */
    ...
    {
        logger.info("Got IQ from jibri: " + iq.toString());
    ...
    {
        System.out.println("TryStartRestartJibri");
        boolean doRetry
        ...
        if (doRetry && retryAttempt++ < NUM_RETRIES)
        {
            ...
            {
                JitsiMeetJson jsonEl = createJibriUpdate(this.sessionId, JibriIq.Status.FAILED);
                Message message = chatRoom.getMuc().createMessage();
                message.addExtension(jsonEl);
                sendChatMessage(message);

                //xmpp.sendStanza(iq);
            }
//            TODO: figure out what to do with this 'error' stuff
//            else if (error == null)
//            {
...
//            }
        ...
        System.out.println("Error starting/restarting jibri, stopping");
        stop(error);
    }
    ...
    private void startJibri(final EntityFullJid jibriJid)
    {
        ...
        System.out.println("Setting session id to " + this.sessionId);
        ...
        // new start requests
        //TODO BRIAN: don't update status here, because we'll wait to get
        // pending back from Jibri. But do we need to update with RETRYING?
        // (since jibri doesn't send that)
...
//            ? JibriIq.Status.RETRYING : JibriIq.Status.PENDING, null);
        ...
        System.out.println("Sending Jibri start iq: " + startIq.toXML());
        //TODO: better way to get the reply?
        new Thread(() -> {
            try
            {
                logger.info("Sending jibri start and waiting for reply");
                JibriIq resp = (JibriIq)xmpp.sendPacketAndGetReply(startIq);
                logger.info("Got reply : " + resp.toString());
...
//        xmpp.sendStanza(startIq);
    ...
    private void setJibriStatus(JibriIq.Status newStatus, XMPPError error)
    {
        ...
        if (newStatus == JibriIq.Status.PENDING) {
            System.out.println("Sending PENDING message from");
            for (StackTraceElement ste : Thread.currentThread().getStackTrace()) {
                System.out.println(ste);
            }
            JitsiMeetJson jsonEl = createJibriUpdate(this.sessionId, newStatus);
            Message message = chatRoom.getMuc().createMessage();
            message.addExtension(jsonEl);
            sendChatMessage(message);
        }
    ...
    private void sendChatMessage(Message message)
    {
        try
        {
            ...
            chatRoom.getMuc().sendMessage(message);
        } catch (SmackException.NotConnectedException | InterruptedException e)
        {
            logger.error("Error sending chat message: " + e.toString());
        }
    }
    ...
    private static class JitsiMeetJson extends AbstractPacketExtension
    {
        public static final String NAMESPACE = "http://jitsi.org/protocol/jibri";
        public static final String ELEMENT_NAME = "jitsi-meet-json";

        JitsiMeetJson(String jsonBody)
        {
    ...
    private static JitsiMeetJson createJibriUpdate(String sessionId, JibriIq.Status status)
    {
        JSONObject jibriUpdate = new JSONObject();
        jibriUpdate.put("session_id", sessionId);
        jibriUpdate.put("status", status.toString());
        return new JitsiMeetJson(jibriUpdate.toJSONString());
    }
